...
import { PolkahatError } from "hardhat/internal/core/errors";
...
export class LedgerProvider extends ProviderWrapperWithChainId {
  ...
  public get eth(): EthWrapper {
    if (this._eth === undefined) {
      throw new PolkahatError(ERRORS.GENERAL.UNINITIALIZED_PROVIDER);
    }
    return this._eth;
  }
  ...
  private async _ethSign(params: any[]): Promise<unknown> {
    if (params.length > 0) {
      ...
      if (address !== undefined) {
        if (data === undefined) {
          throw new PolkahatError(ERRORS.NETWORK.ETHSIGN_MISSING_DATA_PARAM);
        }
  ...
  private async _ethSendTransaction(params: any[]): Promise<unknown> {
    ...
    if (txRequest.gas === undefined) {
      throw new PolkahatError(ERRORS.NETWORK.MISSING_TX_PARAM_TO_SIGN_LOCALLY, {
        param: "gas",
      });
    }
    ...
    if (!hasGasPrice && !hasEip1559Fields) {
      throw new PolkahatError(ERRORS.NETWORK.MISSING_FEE_PRICE_FIELDS);
    }

    if (hasGasPrice && hasEip1559Fields) {
      throw new PolkahatError(ERRORS.NETWORK.INCOMPATIBLE_FEE_PRICE_FIELDS);
    }
