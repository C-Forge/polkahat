...
describe("PolkahatError", () => {
  describe("Type guard", () => {
    it("Should return true for PolkahatErrors", () => {
      assert.isTrue(
        PolkahatError.isPolkahatError(new PolkahatError(mockErrorDescriptor))
      );
    });

    it("Should return false for everything else", () => {
      assert.isFalse(PolkahatError.isPolkahatError(new Error()));
      assert.isFalse(
        PolkahatError.isPolkahatError(
          new NomicLabsHardhatPluginError("asd", "asd")
        )
      );
      assert.isFalse(PolkahatError.isPolkahatError(undefined));
      assert.isFalse(PolkahatError.isPolkahatError(null));
      assert.isFalse(PolkahatError.isPolkahatError(123));
      assert.isFalse(PolkahatError.isPolkahatError("123"));
      assert.isFalse(PolkahatError.isPolkahatError({ asd: 123 }));
    });
  });

  describe("Without parent error", () => {
    it("should have the right error number", () => {
      const error = new PolkahatError(mockErrorDescriptor);
    ...
    it("shouldn't have a parent", () => {
      assert.isUndefined(new PolkahatError(mockErrorDescriptor).parent);
    });

    it("Should work with instanceof", () => {
      const error = new PolkahatError(mockErrorDescriptor);
      assert.instanceOf(error, PolkahatError);
    });
  });
  ...
  describe("With parent error", () => {
    it("should have the right parent error", () => {
      ...
      const error = new PolkahatError(mockErrorDescriptor, {}, parent);
...
describe("HardhatPluginError", () => {
  describe("Type guard", () => {
    ...
    it("Should return false for everything else", () => {
      ...
      assert.isFalse(
        HardhatPluginError.isHardhatPluginError(
          new PolkahatError(ERRORS.GENERAL.NOT_INSIDE_PROJECT)
        )
      );
