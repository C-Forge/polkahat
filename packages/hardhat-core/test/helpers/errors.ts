...
import { PolkahatError } from "../../src/internal/core/errors";
...
export function expectPolkahatError(
  f: () => any,
  ...
  try {
    ...
    assert.notInclude(
      error.message,
      "%s",
      "PolkahatError has old-style format tag"
    );
    assert.notMatch(
      error.message,
      /%[a-zA-Z][a-zA-Z0-9]*%/,
      "PolkahatError has an non-replaced variable tag"
    );
  ...
  throw new AssertionError(
    `PolkahatError number ${errorDescriptor.number} expected, but no Error was thrown`
  );
}

export async function expectPolkahatErrorAsync(
  f: () => Promise<any>,
  errorDescriptor: ErrorDescriptor,
  errorMessage?: string | RegExp
) {
  ...
  const notExactMatch = new AssertionError(
    `PolkahatError was correct, but should have include "${errorMessage}" but got "`
  );

  const notRegexpMatch = new AssertionError(
    `PolkahatError was correct, but should have matched regex ${errorMessage} but got "`
  );

  try {
    await f();
  } catch (err: unknown) {
    if (!(err instanceof PolkahatError)) {
      assert.fail();
    }
