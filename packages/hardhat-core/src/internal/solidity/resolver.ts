...
import { assertHardhatInvariant, PolkahatError } from "../core/errors";
...
export class Resolver {
  ...
  public async resolveImport(
    ...
    if (scheme !== undefined) {
      throw new PolkahatError(ERRORS.RESOLVER.INVALID_IMPORT_PROTOCOL, {
    ...
    if (replaceBackslashes(imported) !== imported) {
      throw new PolkahatError(ERRORS.RESOLVER.INVALID_IMPORT_BACKSLASH, {
        from: from.sourceName,
        imported,
      });
    }
    ...
    if (isAbsolutePathSourceName(imported)) {
      throw new PolkahatError(ERRORS.RESOLVER.INVALID_IMPORT_ABSOLUTE_PATH, {
        from: from.sourceName,
        imported,
      });
    }
    ...
    if (await includesOwnPackageName(imported)) {
      throw new PolkahatError(ERRORS.RESOLVER.INCLUDES_OWN_PACKAGE_NAME, {
        from: from.sourceName,
        imported,
      });
    }
    ...
    try {
      ...
      if (
        PolkahatError.isPolkahatErrorType(
          error,
  ...
  private async _relativeImportToSourceName(
    ...
    if (
      ...
      throw new PolkahatError(ERRORS.RESOLVER.ILLEGAL_IMPORT, {
        from: from.sourceName,
        imported,
      });
    }
