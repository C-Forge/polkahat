...
async function main() {
  ...
  try {
    ...
    if (hardhatArguments.config === undefined && !isCwdInsideProject()) {
      throw new PolkahatError(ERRORS.GENERAL.NOT_INSIDE_PROJECT);
    }
    ...
    if (
      ...
      throw new PolkahatError(ERRORS.GENERAL.NON_LOCAL_INSTALLATION);
    }
    ...
    if (hardhatArguments.help && taskName !== TASK_HELP) {
      ...
      if (taskDefinition === undefined) {
        if (scopeName !== undefined) {
          throw new PolkahatError(ERRORS.ARGUMENTS.UNRECOGNIZED_SCOPED_TASK, {
            scope: scopeName,
            task: taskName,
          });
        }
        throw new PolkahatError(ERRORS.ARGUMENTS.UNRECOGNIZED_TASK, {
          task: taskName,
        });
      }

      if (taskDefinition.isSubtask) {
        throw new PolkahatError(ERRORS.ARGUMENTS.RUNNING_SUBTASK_FROM_CLI, {
    ...
    let isPolkahatError = false;

    if (PolkahatError.isPolkahatError(error)) {
      isPolkahatError = true;
      console.error(
...
async function createNewProject() {
  if (isCwdInsideProject()) {
    throw new PolkahatError(ERRORS.GENERAL.HARDHAT_PROJECT_ALREADY_CREATED, {
  ...
  if (process.platform === "win32") {
    throw new PolkahatError(ERRORS.GENERAL.NOT_INSIDE_PROJECT_ON_WINDOWS);
  }

  throw new PolkahatError(ERRORS.GENERAL.NOT_IN_INTERACTIVE_SHELL);
}
