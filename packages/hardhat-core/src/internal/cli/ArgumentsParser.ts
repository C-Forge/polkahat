...
import { PolkahatError } from "../core/errors";
...
export class ArgumentsParser {
  ...
  public static cLAToParamName(cLA: string): string {
    if (cLA.toLowerCase() !== cLA) {
      throw new PolkahatError(ERRORS.ARGUMENTS.PARAM_NAME_INVALID_CASING, {
        param: cLA,
      });
    }
  ...
  private _parseTaskParamArguments(
    ...
    for (let i = 0; i < rawCLAs.length; i++) {
      ...
      if (!this._isCLAParamName(arg, taskDefinition.paramDefinitions)) {
        throw new PolkahatError(ERRORS.ARGUMENTS.UNRECOGNIZED_PARAM_NAME, {
          param: arg,
        });
      }
  ...
  private _addTaskDefaultArguments(
    ...
    for (const paramName of Object.keys(taskDefinition.paramDefinitions)) {
      ...
      if (!definition.isOptional) {
        throw new PolkahatError(ERRORS.ARGUMENTS.MISSING_TASK_ARGUMENT, {
  ...
  private _parseArgumentAt(
    ...
    if (parsedArguments[paramName] !== undefined) {
      throw new PolkahatError(ERRORS.ARGUMENTS.REPEATED_PARAM, {
        param: claArg,
      });
    }
  ...
  private _parsePositionalParamArgs(
    ...
    for (let i = 0; i < positionalParamDefinitions.length; i++) {
      ...
      if (rawArg === undefined) {
        if (!definition.isOptional) {
          throw new PolkahatError(ERRORS.ARGUMENTS.MISSING_POSITIONAL_ARG, {
    ...
    if (
      ...
      throw new PolkahatError(ERRORS.ARGUMENTS.UNRECOGNIZED_POSITIONAL_ARG, {
