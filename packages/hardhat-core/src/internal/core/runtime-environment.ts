...
import { PolkahatError } from "./errors";
...
export class Environment implements HardhatRuntimeEnvironment {
  ...
  constructor(
    ...
    if (networkConfig === undefined) {
      throw new PolkahatError(ERRORS.NETWORK.CONFIG_NOT_FOUND, {
        network: networkName,
      });
    }
  ...
  public readonly run: RunTaskFunction = async (
    ...
    if (scope === undefined) {
      ...
      if (scopeDefinition === undefined) {
        throw new PolkahatError(ERRORS.ARGUMENTS.UNRECOGNIZED_SCOPE, {
          scope,
        });
      }
   ...
   *
   * @private
   * @throws PolkahatError if any of the following are true:
