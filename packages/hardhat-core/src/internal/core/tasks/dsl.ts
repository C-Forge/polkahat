...
import { PolkahatError, assertHardhatInvariant } from "../errors";
...
export class TasksDSL {
  ...
  public scope(name: string, description?: string): ScopeDefinition {
    if (this._tasks[name] !== undefined) {
      throw new PolkahatError(ERRORS.TASK_DEFINITIONS.TASK_SCOPE_CLASH, {
        scopeName: name,
      });
    }
  ...
  private _addTask<TaskArgumentsT extends TaskArguments>(
    ...
    if (scope === undefined && this._scopes[task] !== undefined) {
      throw new PolkahatError(ERRORS.TASK_DEFINITIONS.SCOPE_TASK_CLASH, {
        taskName: task,
      });
    }
