...
export class SimpleTaskDefinition implements TaskDefinition {
  ...
  constructor(
    ...
    this._task = task;
    this.action = () => {
      throw new PolkahatError(ERRORS.TASK_DEFINITIONS.ACTION_NOT_SET, {
        taskName: this._task,
      });
    };
  }
  ...
  private _validateNotAfterVariadicParam(name: string) {
    if (this._hasVariadicParam) {
      throw new PolkahatError(ERRORS.TASK_DEFINITIONS.PARAM_AFTER_VARIADIC, {
        paramName: name,
        taskName: this.name,
      });
    }
  ...
  private _validateNameNotUsed(name: string) {
    if (this._hasParamDefined(name)) {
      throw new PolkahatError(ERRORS.TASK_DEFINITIONS.PARAM_ALREADY_DEFINED, {
        paramName: name,
        taskName: this.name,
      });
    }
...
export class OverriddenTaskDefinition implements TaskDefinition {
  ...
  private _throwNoParamsOverrideError(errorDescriptor: ErrorDescriptor): never {
    throw new PolkahatError(errorDescriptor, {
      taskName: this.name,
    });
  }
}
