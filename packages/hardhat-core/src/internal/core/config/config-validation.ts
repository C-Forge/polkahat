...
/**
 * Validates the config, throwing a PolkahatError if invalid.
 * @param config
 */
export function validateConfig(config: any) {
  ...
  throw new PolkahatError(ERRORS.GENERAL.INVALID_CONFIG, { errors: errorList });
}
...
export function validateResolvedConfig(resolvedConfig: HardhatConfigT) {
  ...
  for (const run of runs) {
    if (run >= 2 ** 32) {
      throw new PolkahatError(ERRORS.GENERAL.INVALID_CONFIG, {
