...
export class VarsManager {
  ...
  public set(key: string, value: string) {
    ...
    if (value === "") {
      throw new PolkahatError(ERRORS.VARS.INVALID_EMPTY_VALUE);
    }
  ...
  public validateKey(key: string) {
    ...
    if (!KEY_REGEX.test(key)) {
      throw new PolkahatError(ERRORS.VARS.INVALID_CONFIG_VAR_NAME, {
        value: key,
      });
    }
  }
    ...
    for (const key in process.env) {
      if (key.startsWith(this._ENV_VAR_PREFIX)) {
        ...
        if (
          ...
          throw new PolkahatError(ERRORS.ARGUMENTS.INVALID_ENV_VAR_VALUE, {
            varName: key,
            value: envVar!,
          });
        }
