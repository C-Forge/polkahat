...
import { assertHardhatInvariant, PolkahatError } from "../../errors";
...
export function rpcQuantityToBigInt(quantity: string): bigint {
  ...
  if (!isRpcQuantityString(quantity)) {
    throw new PolkahatError(ERRORS.NETWORK.INVALID_RPC_QUANTITY_VALUE, {
      value: quantity,
    });
  }
...
export function rpcDataToBuffer(data: string): Buffer {
  ...
  if (!isRpcDataString(data)) {
    throw new PolkahatError(ERRORS.NETWORK.INVALID_RPC_DATA_VALUE, {
      value: data,
    });
  }
