...
export class PolkahatError extends CustomError {
  public static isPolkahatError(other: any): other is PolkahatError {
    return (
      other !== undefined && other !== null && other._isPolkahatError === true
    );
  }

  public static isPolkahatErrorType(
    other: any,
    descriptor: ErrorDescriptor
  ): other is PolkahatError {
    return (
      PolkahatError.isPolkahatError(other) &&
  ...
  private readonly _isPolkahatError: boolean;
  ...
  constructor(
    ...
    this._isPolkahatError = true;
    Object.setPrototypeOf(this, PolkahatError.prototype);
  }
}
  ...
  if (!isRecursiveCall) {
    for (const variableName of Object.keys(values)) {
      if (variableName.match(/^[a-zA-Z][a-zA-Z0-9]*$/) === null) {
        throw new PolkahatError(ERRORS.INTERNAL.TEMPLATE_INVALID_VARIABLE_NAME, {
      ...
      if (!template.includes(variableTag)) {
        throw new PolkahatError(ERRORS.INTERNAL.TEMPLATE_VARIABLE_TAG_MISSING, {
          variable: variableName,
        });
      }
    }
...
export function assertHardhatInvariant(
  ...
  if (!invariant) {
    throw new PolkahatError(ERRORS.GENERAL.ASSERTION_ERROR, { message });
  }
}
