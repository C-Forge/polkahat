...
export class Artifacts implements IArtifacts {
  ...
  public async artifactExists(name: string): Promise<boolean> {
    ...
    try {
      ...
      if (PolkahatError.isPolkahatError(e)) {
        return false;
      }
   ...
   * @throws {PolkahatError} If the name is not fully qualified.
   */
   ...
   *
   * @throws {PolkahatError} with descriptor:
  ...
  private async _getValidArtifactPathFromFullyQualifiedName(
    ...
    try {
      ...
      if (artifactPath !== trueCasePath) {
        throw new PolkahatError(ERRORS.ARTIFACTS.WRONG_CASING, {
...
`;
  ...
  /**
   * @throws {PolkahatError} with a list of similar contract names.
   */
  private _handleWrongArtifactForFullyQualifiedName(
    ...
    throw new PolkahatError(ERRORS.ARTIFACTS.NOT_FOUND, {
  ...
  private _getArtifactPathFromFiles(
    ...
    if (matchingFiles.length > 1) {
      ...
      throw new PolkahatError(ERRORS.ARTIFACTS.MULTIPLE_FOUND, {
        contractName,
