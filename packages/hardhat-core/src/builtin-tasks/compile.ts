...
import { assertHardhatInvariant, PolkahatError } from "../internal/core/errors";
    ...
    async ({ absolutePath }: { absolutePath: string }): Promise<string> => {
      try {
        ...
        if (fsExtra.lstatSync(absolutePath).isDirectory()) {
          throw new PolkahatError(ERRORS.GENERAL.INVALID_READ_OF_DIRECTORY, {
            absolutePath,
          });
        }
  ...
  .setAction(
    async (
      ...
      try {
        ...
        throw new PolkahatError(ERRORS.BUILTIN_TASKS.COMPILE_FAILURE);
      }
    }
  );
  ...
  .setAction(
    async ({
      ...
      if (solcVersion !== undefined && semver.valid(solcVersion) === null) {
        throw new PolkahatError(ERRORS.ARGUMENTS.INVALID_VALUE_FOR_TYPE, {
          value: solcVersion,
          name: "solcVersion",
          type: "string",
        });
