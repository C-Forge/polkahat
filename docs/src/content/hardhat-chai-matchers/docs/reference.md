...
When `wookashwackomytest-polkahat-chai-matchers` is used, equality comparisons of numbers will work even if the numbers are represented by different types. This means that assertions like this:
...
```ts
import { PANIC_CODES } from "wookashwackomytest-polkahat-chai-matchers/panic";
...
```ts
import { anyValue } from "wookashwackomytest-polkahat-chai-matchers/withArgs";
